...
**File:** `sejm_analysis/sejm_asw_real_analysis.json`  
**Type:** general_change  
**Detected at:** 2025-11-04T19:00:00.564289  

**File Preview:**
```
{
  "committee": {
    "appointmentDate": "2019-11-13",
    "code": "ASW",
    "compositionDate": "2019-11-13",
    "members": [
      {
        "club": "Lewica",
      ...
      {
        "club": "PiS",
        "function": "zastępca przewodniczące
...
```
...
1. **Analyze the change:**
   - Read the full file: `sejm_analysis/sejm_asw_real_analysis.json`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%sejm_analysis/sejm_asw_real_analysis.json%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'sejm_analysis/sejm_asw_real_analysis.json' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "sejm_analysis/sejm_asw_real_analysis.json"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*sejm_asw_real_analysis*"
# Should return > 0
```
...
**Task:** Propagate sejm_analysis/sejm_asw_real_analysis.json to databases  
