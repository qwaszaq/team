...
**File:** `POST_EVALUATION_PROCESS.md`  
**Type:** process_change  
**Detected at:** 2025-11-03T17:34:51.817962  
...
1. **Analyze the change:**
   - Read the full file: `POST_EVALUATION_PROCESS.md`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%POST_EVALUATION_PROCESS.md%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'POST_EVALUATION_PROCESS.md' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "POST_EVALUATION_PROCESS.md"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*POST_EVALUATION_PROCESS*"
# Should return > 0
```
...
**Task:** Propagate POST_EVALUATION_PROCESS.md to databases  
