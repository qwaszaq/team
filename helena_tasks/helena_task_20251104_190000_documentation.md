...
**File:** `docs/research/SEJM_ASW_ANALYSIS_2019_2023.md`  
**Type:** documentation  
**Detected at:** 2025-11-04T19:00:00.564276  
...
**Źródło Danych:** https://api.sejm.gov.pl

---

## 📋 Executive Summary

Kompleksowa analiza pracy Komisji Administracji i Spraw Wewnętrznych w IX kadencji Sejmu (2019-2023) na podstawie danych z oficjalnego API Sejmu RP.
...
1. **Analyze the change:**
   - Read the full file: `docs/research/SEJM_ASW_ANALYSIS_2019_2023.md`
...
7. **Report:**
   ...
   - Save as: `/Users/artur/coursor-agents-destiny-folder/helena_tasks/completed_20251104_190000.md`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%docs/research/SEJM_ASW_ANALYSIS_2019_2023.md%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'docs/research/SEJM_ASW_ANALYSIS_2019_2023.md' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "docs/research/SEJM_ASW_ANALYSIS_2019_2023.md"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*SEJM_ASW_ANALYSIS_2019_2023*"
# Should return > 0
```
...
**Task:** Propagate docs/research/SEJM_ASW_ANALYSIS_2019_2023.md to databases  
