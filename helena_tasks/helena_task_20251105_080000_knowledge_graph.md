...
**File:** `sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher`  
**Type:** knowledge_graph  
**Detected at:** 2025-11-05T08:00:00.583512  
...
1. **Analyze the change:**
   - Read the full file: `sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*neo4j_20251104_220923_BATCH_MIGRATION_PLAN*"
# Should return > 0
```
...
**Task:** Propagate sql/realtime_updates/neo4j_20251104_220923_BATCH_MIGRATION_PLAN.cypher to databases  
