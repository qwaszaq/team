...
**File:** `sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql`  
**Type:** process_change  
**Detected at:** 2025-11-04T11:00:00.602179  
...
1. **Analyze the change:**
   - Read the full file: `sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql`
...
```sql
-- PostgreSQL check
SELECT COUNT(*) FROM team_tools WHERE file_path LIKE '%sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql%';
-- Should return > 0

-- Neo4j check
MATCH (n) WHERE n.file_path CONTAINS 'sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql' RETURN count(n);
-- Should return > 0
```
...
curl -X POST http://localhost:6333/collections/destiny-team-framework-master/points/scroll \
  ...
  -d '{"filter": {"must": [{"key": "file_path", "match": {"text": "sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql"}}]}}' | jq '.result.points | length'
# Should return > 0

# Redis check
docker exec kg-redis redis-cli KEYS "*pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104*"
# Should return > 0
```
...
**Task:** Propagate sql/realtime_updates/pg_20251104_100330_HELENA_PROCESSING_REPORT_20251104.sql to databases  
